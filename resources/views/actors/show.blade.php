            ...
            <div class="flex-none">
                ...
                <ul class="flex items-center mt-4">
                    ...
                    @if ($social['facebook_id'])
                        <li>
                            <a target="_blank" href={{"https://www.facebook.com/" . $social['facebook_id']}} title="Facebook">
                    ...
                    @if ($social['instagram_id'])
                        <li class="ml-6">
                            <a target="_blank" href={{"https://www.instagram.com/" . $social['instagram_id']}} title="Instagram">
                    ...
                    @if ($social['twitter_id'])
                        <li class="ml-6">
                            <a target="_blank" href={{"https://www.twitter.com/" . $social['twitter_id']}} title="Twitter">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ $actor['birthday'] }}
                       ( {{\Carbon\Carbon::parse($actor['birthday'])->diff(\Carbon\Carbon::now())->format('%y ' )}} years old )
                        in {{ $actor['place_of_birth'] }}</span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    @foreach ($knownFor as $movie)
                        <div class="mt-4">
                            <a href="{{route('movies.show' , $movie['id'])}}"><img src="{{ 'https://image.tmdb.org/t/p/w500' . $movie['poster_path']}}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
                            <a href="{{route('movies.show' , $movie['id'])}}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $movie['title'] }}</a>
                            <span class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">Known As : {{$movie['character']}}</span>
                        </div>
